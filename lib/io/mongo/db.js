...
function getOptionPrices(optionName, day, callback) {
...
function getMinuteBarNum(symbolName, day, minuteNum, callback) {
	...
	Bar.find(filters).exec(function(err, result) {
console.log("where", symbolName, day, minuteNum)
		if (err) {
			console.log('returned error');
			return callback(err);
		}
...
function getAvailableOptionNames(symbolName, day, callback) {

	var nextDay = util.getNextDay(day);
	var re = new RegExp("^" + symbolName + "[0-9](.*)");
	var filters = {
		"symbolName": { "$regex": re, "$options": "i" },
		"dateTime": {"$gte": day, "$lt": nextDay}
	};

	Bar.find(filters).distinct("symbolName").exec(function(err, result) {
		if (err) {
			console.log('getAvailableOptionNames returned error');
			return callback(err);
		}
		callback(null, result);
	});
