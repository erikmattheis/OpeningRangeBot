...
var symbolNames;
var lastPrices = [];
var checkIfUpInterval;
exports.checkIfUpInterval = checkIfUpInterval;
...
function destroy() {
	if (connection && connection.destroy) {
		...
		console.log("closing connection to TradeKing");

		clearInterval(checkIfUpInterval);
		connection.end();
		connection.abort();
		connection.destroy();
...
function openConnection(callback) {
	console.log("OPENING CONNECTIOM")
	if (connection) {
	...
	openConnection(function(err, connection) {
		...
		connection.on('response', function (response) {
			...
			if (response.statusCode !== 200) {
				...
				console.log("TradeKing stream initiated for ", symbolNames.length, " symbols");
...
function init(symbols, recordOptions, callback) {
	...
	if (!recordOptions) {
		...
		checkIfUpInterval = setInterval(tryToReconnectIfThereHasBeenNoActivity, 5000);
	}
	else {
		getQuotes(symbols, function(err, result) {
			if (err) { console.log(err);}
			var i = 0;
			async.whilst(
				function () { return i < result.quotes.quote.length; },
				function (next) {
					var range = (["AAPL"].indexOf(result.quotes.quote[i].symbol) === -1) ? .51 : 1.01;
					var strikeLow = Math.floor(result.quotes.quote[i].last) - range;
					var strikeHigh = Math.floor(result.quotes.quote[i].last) + range;
					getOptions(result.quotes.quote[i].symbol, new Date(), strikeLow, strikeHigh, function(err, options) {

						for (var j = 0; j < options.quotes.quote.length; j++) {
							symbolNames.push(options.quotes.quote[j].symbol);
						}
...
function getOptions(symbolName, expirationDate, strikeLow, strikeHigh, callback) {
	...
	tradeking_consumer.get(uri,
		...
		function(error, data, response) {
			if (error) return console.log('tradeking_consumer.get2', error);
...
function getQuotes(symbolNames, callback) {
	...
	var uri = "https://api.tradeking.com/v1/market/ext/quotes.json?symbols="
		+ symbolNames.join(",").toUpperCase();
