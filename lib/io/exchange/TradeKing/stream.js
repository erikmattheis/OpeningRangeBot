...
function buyOptions(trade, callback) {
	...
	getOptions(trade.symbol, new Date(), trade.entry - 1, trade.entry + 1, function(err, result) {
		if (err) return logger.log("buy stream.getOptions-error", trade);
		...
		console.log('First option of ', trade.position, ' ', trade.entry, ' is ', n);
		return n;
...
function sellOptions(trade, callback) {
	...
	getOptions(trade.symbol, new Date(), trade.entry - 1, trade.entry + 1, function(err, result) {
		/* TODO: we want to do more than log the error */
		if (err) {
