...
function dummyData() {
		...
		var d = '{"trade":{"symbol":"AAPL", "last":100}}  {"trade":{"symbol":"IWM", "last":120}} {"quote":{"symbol":"IWM", "ask":120, "bid":119}}';
	consumeData(d);
}
...
function pump() {
	...
	while ((pos = buffer.indexOf('}}')) >= 0) {
		emittData(buffer.slice(0, pos + 2));
	...
	if (data.trade) {
		...
		var bar = {
			...
			*/
			type: "quote",
			exchangeId: 3,
			symbolName: data.quote.symbol,
			dateTime: new Date(parseInt(data.quote.timestamp)),
