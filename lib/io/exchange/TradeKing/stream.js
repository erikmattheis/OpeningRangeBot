...
function emittData(data) {
	/*data = '{"trade":{"cvol":"37725135","datetime":"2015-03-17T14:08:11-04:00","exch":"The Trade Reporting Facility LLC","last":"127.2378","symbol":"AAPL","tcond":"9","timestamp":"1426615691","vl":"5","vwap":"126.2358"}}';*/

	try {
		data = JSON.parse(data);
	}
	catch (e) {
		return console.log("error", numErrors++);
	}
	...
	if (data.trade) {
		...
		var bar = {
			...
			*/
			...
			dateTime: new Date(parseInt(data.trade.timestamp)),
			timestamp: data.trade.timestamp,
			last: data.trade.last,
			volume: data.trade.vl
		}
