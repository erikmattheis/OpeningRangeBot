...
SPY 0.03 avg trade per day
 2.83 total profit
 0.6407766990291263 w/l

largest win: 0.762599999999992
largest loss: -0.18500000000000227
QQQ 0.02 avg trade per day
 2.47 total profit
 0.6407766990291263 w/l

largest win: 0.710000000000008
largest loss: -0.10999999999999943
IWM 0.03 avg trade per day
 3.31 total profit
 0.6213592233009708 w/l

largest win: 0.6099999999999994
largest loss: -0.2704000000000093
FB 0.02 avg trade per day
 1.49 total profit
 0.5510204081632653 w/l

largest win: 1.1400000000000006
largest loss: -0.3499999999999943
AAPL 0.02 avg trade per day
 1.54 total profit
 0.4897959183673469 w/l

largest win: 1.0100000000000051
largest loss: -0.3119999999999976
simulation complete
 */
...
console.log("_____________")
	...
	symbolToday.opens.push(data.open);
	symbolToday.closes.push(data.close);

	// TODO: should start entirely new symbol object each new day instead
	symbolToday.date = data.dateTime;

	// TODO: WTF is going on? Why do I have to do this?
	...
	if ((data.dateTime.getHours() > 8
		...
		// TODO: allow trader to buy and sell during same minute
			...
			if (data.low <= symbolToday.stopLoss) {
				...
				if (symbolToday.stopLoss < symbolToday.purchasePrice) {
			...
			if (data.high >= symbolToday.stopLoss) {
				...
				symbolToday.salePrice = symbolToday.stopLoss > data.open ? symbolToday.stopLoss : data.open;
				symbolToday.profit = symbolToday.purchasePrice - symbolToday.salePrice;
				symbols[data.symbolName].profit += symbolToday.profit;
				util.thinkOutLoud("closed position on stop loss");
			}
			else if (data.high <= symbolToday.profitTarget) {
			...
			if (!lowerLows && higherHighs
				...
				var projectedGain = (symbolToday.profitTarget - symbolToday.openAt);
				projectedGain = config.maxLoss < projectedGain * config.riskFactor ? config.maxLoss : projectedGain * config.riskFactor;
				symbolToday.stopLoss = symbolToday.openAt - projectedGain;
				...
				var projectedGain = (symbolToday.openAt - symbolToday.profitTarget);
				projectedGain = config.maxLoss > projectedGain * config.riskFactor ? config.maxLoss : projectedGain * config.riskFactor;
				symbolToday.stopLoss = symbolToday.openAt + projectedGain;
		...
		symbols[data.symbolName].largestWin = symbolToday.profit > symbols[data.symbolName].largestWin ? symbolToday.profit : symbols[data.symbolName].largestWin;
		symbols[data.symbolName].largestLoss = symbolToday.profit < symbols[data.symbolName].largestLoss ? symbolToday.profit : symbols[data.symbolName].largestLoss;
	}
}
...
function closeLongOrAdjustOrders(symbolName, data, forceClose) {
	...
	if (!forceClose && !lowerLows && higherHighs) {
		symbolToday.profitTarget = extendTrend(symbolToday.highs);
		var newStopLoss = data.close - ((symbolToday.profitTarget - data.close) * config.riskFactor);
		symbolToday.stopLoss = newStopLoss > symbolToday.stopLoss ? newStopLoss : symbolToday.stopLoss;
