	...
	if ((data.dateTime.getHours() >= 8
		...
		if (symbols[todayString][data.symbolName].position
			...
			if (symbols[todayString][data.symbolName].position === "LONG"
				...
				symbols[todayString][data.symbolName].purchasePrice = data.open < symbols[todayString][data.symbolName].openAt ? data.open : symbols[todayString][data.symbolName].openAt;
				util.thinkOutLoud("My order to go long triggered at minute " + minuteNum + " at " + symbols[todayString][data.symbolName].purchasePrice + " - low was " + data.low);
			}
				...
				symbols[todayString][data.symbolName].openedAtMinuteNum = minuteNum;
				symbols[todayString][data.symbolName].purchasePrice = data.open > symbols[todayString][data.symbolName].openAt ? data.open : symbols[todayString][data.symbolName].openAt;
				util.thinkOutLoud("My order to go short triggered at minute " + minuteNum + " at " + symbols[todayString][data.symbolName].purchasePrice);
			}
		}
		...
		if (!symbols[todayString][data.symbolName].hasOpened) {
			if (!lowerLows && higherHighs) {
				...
				util.thinkOutLoud("projecting next minute's low to be: " + symbols[todayString][data.symbolName].openAt);
...
function closeLongOrAdjustOrders(symbolName, data, forceClose) {
	...
	if (!forceClose && !lowerLows && higherHighs) {
		...
		symbols[data.todayString][symbolName].profit += symbols[data.todayString][data.symbolName].salePrice - symbols[data.todayString][symbolName].purchasePrice;
