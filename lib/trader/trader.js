...
function sell(trade, data, type) {
	/* TODO: rename buyOptions to something accurate */
	broker.buyOptions(trades[trade.symbol], function(err, result) {
		if (err) return logger.log("option-purchase-error", trade);
		/* TODO: this is always the ask - make it give the bid when selling */
		trades[trade.symbol].optionExitPrice = result.optionPrice;
