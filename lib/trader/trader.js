...
function think(data) {
	...
	if (data.type === "trade") {
		...
		*/
		...
		// TODO: following line gave openingRange[data.symbolName] is undefined error. How????
		if (data.low > openingRange[data.symbolName].high) {
			trades[data.symbolName] = {
				position: "LONG",
				symbol: data.symbolName,
				stopLoss: config.openingRangeStopLossFactor * (openingRange[data.symbolName].high - openingRange[data.symbolName].low),
				profitTarget: config.openingRangeTakeProfitFactor * (openingRange[data.symbolName].high - openingRange[data.symbolName].low),
