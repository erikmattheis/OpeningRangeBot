...
var targetGranularity = 0.0003;
var smallestAbsoluteFraction = 0.0003;
var largestAbsoluteFraction = 0.006;
var numMinutes = 10;
	...
	async.waterfall([
		...
		function(closestOptionPrice, callback) {
			...
			if (!optionBarsCache[putName]) {
				optionBarsCache[putName] = [];
			}
			if (optionBarsCache[putName][buyTime]) {
				return callback(null, callName, optionBarsCache[putName][buyTime])
			}
			db.getOptionPriceAtTime(putName, buyTime, function(err, putData) {
				if (err) { throw new Error(err)};

				
				optionBarsCache[putName][buyTime] = putData;
				callback(null, callName, putData);
			});
    	},
		function(callName, putData, callback) {
			if (!optionBarsCache[callName]) {
				optionBarsCache[callName] = [];
			}
			if (optionBarsCache[callName][buyTime]) {
				return callback(null, {putData: putData, callData: optionBarsCache[callName][buyTime]})
			}
			db.getOptionPriceAtTime(callName, buyTime, function(err, callData) {
				if (err) { throw new Error(err)};

				optionBarsCache[callName][buyTime] = callData;
