...
function getOHLCsForDay(symbolName, date, callback) {
	...
	function (err, result) {
		...
		var straddleOHLCs = getStraddlePrice(result, date);
...
function getStraddlePrice(arr, date) {
	var straddlePrices = [];
	if (!arr
		|| (!arr.puts || !arr.puts.length)
		|| (!arr.calls || !arr.calls.length)) {
		return [];
...
*/
	...
	simulateDates(new Date(2015, 1, 3), endSimulationOn, null, function(err) {
				var logStr = "<tr>"
			+ "<td>" + capital + "</td>"
...
function getTrades(symbolName, day, callback) {
	var buyTime = util.minuteNumToTime(config.buyAtMinute, day);
	
	straddle(symbolName, buyTime, function(err, result) {
...
function simulateDates(from, to, mostRecentTrade, callback) {
	...
	if (day.getDay() === 6 || day.getDay() === 0) {
		...
		return simulateDates(day, to, mostRecentTrade, callback);
	}
...
function simulate(data, buyTime, callback) {
	...
	for (var k = start; k < combinedPrices.putData.length; k++) {
		if (!combinedPrices.putData[k] || !combinedPrices.callData[k]) {
			...
			trade.combinedSalePrice = combinedPrices.callData[k-1].low + combinedPrices.putData[k-1].low;
			trade.profit = trade.combinedSalePrice - trade.combinedPurchasePrice;
			trade.putSaleTime = combinedPrices.putData[k-1].dateTime;
		...
		trade.combinedSalePrice = combinedPrices.callData[k].low + combinedPrices.putData[k].low;

		trade.putSaleTime = combinedPrices.putData[k].dateTime;
