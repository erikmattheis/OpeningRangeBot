...
function startSimulation(exchangeId) {
	if (config.runSimulation === true) {
		simulator.init(exchangeId, function(err, emitter, broker) {
			if (err) return console.log("error:", err);
			trader.Trader(emitter, broker);
			simulator.simulateDates(new Date(2015, 2, 10), new Date(2015, 2, 15));
		});
	}
}
...
function recordData(bar, callback) {
	...
	db.createBar(bar, function(err, result) {
		if (err) {
			...
			console.log("recorded ", result.symbolName, " bar. #", num++, " at ", result.dateTime.getHours(), ":", result.dateTime.getMinutes(), result.type);
			if (callback) {
				callback(null);
			}
		}
