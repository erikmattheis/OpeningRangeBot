...
function startStream(callback) {
	if (config.streamSource === "TradeKing") {
		stream.init(["IWM", "SPY", "AAPL", "QQQ", "DIA", "FB"], function(err, emitter) {

			if (err) return console.log("error:", err);

			emitter.on("data", recordData);

			trader.Trader(emitter);
		...
		simulator.init(new Date(2015, 3, 30), function(err, emitter) {
		...
		csvData.init(function(err, result) {
			...
			function(err){
				if( err ) {
					...
					callback();
					/*
					db.getAllBarsBySymbol("IWM", function(err, result) {
						console.log(result[900]);
						console.log(result[901]);
						console.log(result[902]);
					})
			*/
				}
			});
